"""V11 statistical validation — confidence intervals, permutation tests, regime analysis.

All tests are dependence-aware and pre-registered.
"""
...
def compute_net_ev_series(
    ...
    """Compute per-order net EV series."""
    filled = (fill_probabilities > 0.5).astype(float)
    gross = filled * predicted_returns
...
) -> V11StatisticalResult:
    """Run full statistical validation on net EV series.

    Args:
        net_ev: Per-order net EV in bps
        alpha: Significance level
        n_permutations: Number of permutation iterations

    Returns:
        V11StatisticalResult with all test outcomes
    """
    x = net_ev.dropna().values
    n = len(x)
    if n == 0:
        return V11StatisticalResult(
...
def analyze_regimes(
    ...
    if "spread_bps" in df.columns:
        ...
        regimes["spread_regime"] = {
            "high_spread": {
                "n": len(high_spread),
                "mean_net_ev_bps": float(high_spread.mean()) if len(high_spread) > 0 else None,
                "positive_rate": float((high_spread > 0).mean()) if len(high_spread) > 0 else None,
            },
            "low_spread": {
                "n": len(low_spread),
                "mean_net_ev_bps": float(low_spread.mean()) if len(low_spread) > 0 else None,
                "positive_rate": float((low_spread > 0).mean()) if len(low_spread) > 0 else None,
            },
        }
    ...
    if "vol_500" in df.columns:
        ...
        regimes["vol_regime"] = {
            "high_vol": {
                "n": len(high_vol),
                "mean_net_ev_bps": float(high_vol.mean()) if len(high_vol) > 0 else None,
                "positive_rate": float((high_vol > 0).mean()) if len(high_vol) > 0 else None,
            },
            "low_vol": {
                "n": len(low_vol),
                "mean_net_ev_bps": float(low_vol.mean()) if len(low_vol) > 0 else None,
                "positive_rate": float((low_vol > 0).mean()) if len(low_vol) > 0 else None,
            },
        }
