...
class ExecutionGateway:
    ...
    def _best_effort_cancel_all(self, symbol: str) -> None:
        cancel_all = getattr(self.adapter, "cancel_all", None)
    ...
    def submit(self, submission: Submission) -> ExecutionResult:
        ...
        try:
            ...
            self._best_effort_cancel_all(submission.symbol)
    ...
    def cancel_all(self, symbol: str) -> ExecutionResult:
        """Cancel all exchange orders for a symbol, then clean local state."""
        if not self.live_enabled:
            return ExecutionResult(
                "BLOCKED_LIVE_DISABLED",
                None,
        ...
        if cancel_all is None:
            ...
            return ExecutionResult(
                ...
                None,
                "exchange adapter does not implement cancel_all",
            )
        try:
            ...
            return ExecutionResult(
                "CANCEL_ALL_UNKNOWN",
                None,
                f"cancel-all outcome unknown: {type(exc).__name__}",
            )
        self.manager.emergency_close_all(reason="exchange_cancel_all")
        self.risk_gate.update_orders(len(self.manager.open_orders))
        return result
