"""V15 production gate — hard lock. All gates must pass or LIVE stays disabled."""
...
class V15ProductionGate:
    ...
    def evaluate(self) -> dict:
        ...
        model_path = ARCHIVE / "v15_frozen_model.joblib"
        frozen_artifact = _load_json(EVIDENCE / "v15_calibration.json")
        ...
        checks["frozen_artifact_integrity"] = {"status": "PASS" if model_sum == stored_sum and stored_sum else "FAIL"}
        ...
        forward_result = _load_json(EVIDENCE / "v15_forward_validation.json")
        if not forward_result:
            ...
            n_pass = (
                ...
                and forward_result.get("n_trades", 0) >= 50
            )
            checks["forward_validation"] = {
                "status": "PASS" if n_pass else "FAIL",
                "detail": f"net_ev={forward_result.get('net_ev_bps', 0):.2f} ci_lo={forward_result.get('ci_lower_bps', 0):.2f} p={forward_result.get('p_value', 1):.4f} regimes={forward_result.get('positive_regimes', 0)}/6 trades={forward_result.get('n_trades', 0)}",
            }

        # 5. Execution cost validation
        checks["execution_cost_validation"] = {
            "status": "PASS" if cfg.total_roundtrip_cost_bps < 5.0 else "FAIL",
            "detail": f"total_cost={cfg.total_roundtrip_cost_bps:.2f} bps",
        }

        # 6. Robustness
        checks["robustness"] = {"status": "BLOCKED", "reason": "not implemented for V15"}
        ...
        # 8. Paper trading
        paper_path = ARCHIVE / "v15_paper_trading_result.json"
        ...
        # 10. Config integrity
        checks["config_integrity"] = {"status": "PASS" if cfg.config_hash() == "aa505ca8abe09376" else "FAIL", "detail": cfg.config_hash()}
        ...
        Path("data/evidence/v15_production_gate.json").write_text(json.dumps(result, indent=2, default=str))
        return result
