"""V16 production gate — hard lock. All gates must pass or LIVE stays disabled."""
...
class V16ProductionGate:
    ...
    def evaluate(self) -> dict:
        ...
        if cal_manifests and fwd_manifests:
            ...
            cal_end = cal_m.get("end_ns")
            fwd_start = fwd_m.get("start_ns")
        ...
        return_sum = _file_checksum(return_model_path) if return_model_path.exists() else ""
        fill_sum = _file_checksum(fill_model_path) if fill_model_path.exists() else ""
        ...
        checks["frozen_artifact_integrity"] = {"status": "PASS" if return_sum == stored_return_sum and fill_sum == stored_fill_sum and stored_return_sum else "FAIL"}
        checks["frozen_artifact_immutable"] = {"status": "PASS" if return_model_path.exists() and (oct(return_model_path.stat().st_mode)[-3:] == "444") else "BLOCKED"}
        ...
        if not forward_result:
            ...
            n_pass = (
                ...
                and forward_result.get("n_trades", 0) >= cfg.min_trades_forward
            )
            checks["forward_validation"] = {
                "status": "PASS" if n_pass else "FAIL",
                "detail": f"net_ev={forward_result.get('net_ev_bps', 0):.2f} ci_lo={forward_result.get('ci_lower_bps', 0):.2f} p={forward_result.get('p_value', 1):.4f} trades={forward_result.get('n_trades', 0)}",
            }
        ...
        checks["robustness"] = {"status": "BLOCKED", "reason": "not implemented for V16"}
